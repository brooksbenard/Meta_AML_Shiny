...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Documentation and Downloads", value = "documentation",
      div(class = "welcome-page", style = "max-width: 800px; margin: 0 auto; padding: 24px 15px;",
        h2("Documentation and Downloads"),
        p("Summary tables used by Meta AML Explorer. Generated by ", code("build_aggregated_clinical.R"), " and ", code("build_aggregated_maf.R"), "."),
        wellPanel(
          h4(style = "margin-top: 0;", "Meta AML aggregated clinical"),
          p(style = "margin-bottom: 8px; color: #555;", "One row per sample: cohort, demographics, ELN 2017/2022 risk, survival, and key clinical variables."),
          p(style = "margin-bottom: 8px;", strong("File size:"), textOutput("doc_clinical_size", inline = TRUE)),
          downloadButton("download_agg_clinical", "Download Meta_AML_aggregated_clinical.tsv")
        ),
        wellPanel(
          h4(style = "margin-top: 0;", "Meta AML aggregated mutations (MAF)"),
          p(style = "margin-bottom: 8px; color: #555;", "MAF-style mutation table: one row per variant across all cohorts (Hugo_Symbol, sample, VAF, variant type, cohort, etc.)."),
          p(style = "margin-bottom: 8px;", strong("File size:"), textOutput("doc_maf_size", inline = TRUE)),
          downloadButton("download_agg_maf", "Download Meta_AML_aggregated_mutations.maf.tsv")
        )
      )
    )
  )
